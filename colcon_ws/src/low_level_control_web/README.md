# low_level_control_web

A ROS 2 Python package for **real-time low-level motor status monitoring & control** of the Unitree Go2 robot via a web dashboard.
...
```bash
cd <workspace>
colcon build --packages-select low_level_control_web
...
```bash
source install/setup.bash
ros2 launch low_level_control_web low_level_control_web.launch.py
```
...
| `--port` | `8083` | Web server TCP port |

---

## Dashboard

Open `http://<robot-or-jetson-ip>:8083` in any browser.
...
low_level_control_web/
├── launch/
│   └── low_level_control_web.launch.py   # ROS 2 launch file
├── low_level_control_web/
│   ├── __init__.py
│   ├── main.py                          # Entry point
